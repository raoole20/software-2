...
export async function getDetailsHorasRegistro(registroId: number) {
    try {
        const session = await getSession();
        const response = await request.get<Hours>(`/api/records/registros-horas/${registroId}/`, {
            headers: {
                'Authorization': `Token ${session?.accessToken}`
            }
        });
        ...
        if (error instanceof AxiosError) {
            console.error('Error fetching registro horas details:', error.response?.data || error.message);
            return {
...
export async function aprobarRechazarHorasRegistro(
    ...
    const session = await getSession();
    try {
        const response = await request.post(`/api/records/registros-horas/${registroId}/aprobar_rechazar/`, data, {
            headers: {
                'Authorization': `Token ${session?.accessToken}`
            }
        });
        ...
        if (error instanceof AxiosError) {
            console.error('Error aprobando/rechazando registro horas:', error.response?.data || error.message);
            return {
...
export async function getAllHorasRegistros() {
    ...
    try {
        ...
        if (error instanceof AxiosError) {
            console.error('Error obteniendo registros de horas:', error.response?.data || error.message);
            return {
...
export async function cambiarEstadoRegistroHoras(registro: Hours, nuevoEstado: 'A' | 'R') {
    const session = await getSession();
